...
from ours.data_partition import partition_data
import torch
import os
...
def nlpdataset_read(dataset, base_path, batch_size, n_parties, partition, beta, skew_class):
    if dataset == "yahoo_answers":
        traindata = torch.load(os.path.join(base_path, 'TRAIN_data.pth.tar'))
        testdata = torch.load(os.path.join(base_path, 'TEST_data.pth.tar'))
